...
of \(n\) actors.  Furthermore, some queries calculating top-fifteen lists of
...
%% rating and the top-fifteen queries cannot be evaluated with the test models.
...
\subsection{Extension Task 1: Compute Top-Fifteen Couples (TH)}

The goal of this task is to produce top-fifteen lists of the couples created by
Task~2.  For this purpose, two model queries should be given.

\begin{enumerate}
\item[(a)] Compute the top-fifteen couples according to the average rating of
  their common movies (requires Task~3 to be solved).
\item[(b)] Compute the top-fifteen couples according to the number of common
  movies.
\end{enumerate}

Each of the couples in the top-fifteen lists should be printed with the names of
...
\subsection{Extension Task 4: Compute Top-Fifteen Cliques (TH)}

This is a variant of Extension Task~1 for cliques instead of couples.  Again,
two queries should be given.

\begin{enumerate}
\item[(a)] Compute the top-fifteen cliques of a given size \(n\) according to the
  average rating of their common movies (requires Extension Task~3 to be
  solved).
\item[(b)] Compute the top-fifteen cliques of a given size \(n\) according to the
...
\section{Expected Top-Fifteen Lists}
