...
has been completely solved and the implementation is correct.

The performance of a solution is not relevant, here.  If a solution works
correctly for the smaller models but won't terminate or run out of memory for
the larger models, it may still achieve three points.
...
\item[Task~2] The correct number of couples will be evaluated for both the
  ...
  setting of the \p{p1}, \p{p2}, and \p{commonMovies} references will be spot
  checked.
...
\item[Extension~Task~1] The Top-15 lists of couples will be evaluated for both
  ...
  Sorting stably according criterion (a) and (b) is enough.
...
\item[Extension~Task~4] The Top-15 lists of cliques will be evaluated for both
  ...
  (b) number of common movies is enough.
\end{description}
