...
point can be earned by cutting those ties with the respective other metric.
E.g., when computing metric (a) and two couples have the same average rating,
then the one with more common movies should be listed first.  If both values
are equal, the order may be arbitrary (but preferrably still stable) again.
